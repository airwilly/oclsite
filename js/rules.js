/* ============================================================
   js/rules.js
   Fetches data/Rules.MD, parses it into HTML, and injects it
   into pages/rules.html.

   Markdown features this parser handles:
     Block-level:
       # Heading 1    — skipped (page has its own heading)
       ## Heading 2   — rendered as <h3> (page <h2> is the section title,
                        so rule headings sit one level below at <h3>)
       ---            — <hr> separator
...
const RULES_PATH = '../data/Rules.MD';


/* ------------------------------------------------------------
   init()
   Fetches the rules file and triggers parsing and rendering.
   ------------------------------------------------------------ */
async function init() {
  try {
    const text = await fetch(RULES_PATH).then(res => res.text());
...
function parseMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    /* ── H2 heading (##): render as <h3> ──
       Using <h3> because the page's own section label is <h2>,
       so rule titles sit one level below in the document outline. */
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      while (i < lines.length && /^\d+\.\s/.test(lines[i].trim())) {
        /* Remove the leading "N. " so only the item text remains */
